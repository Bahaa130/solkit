...
const findEligiblePayment = async (solanaRpcUrl, userWallet, hasReferrer) => {
    ...
    try {
        const sigsResp = await withTimeout(connection.getSignaturesForAddress(new PublicKey(ADMIN_WALLET), { limit: 30 }), 8000);
        if (!sigsResp)
            return null;
        const signatures = sigsResp;
        ...
        const results = await Promise.all(batch.map(async (sig) => {
            if (sig.err)
                return null;
            try {
                const tx = await withTimeout(connection.getTransaction(sig.signature, {
...
router.post("/cards/upgrade", authenticateJWT, async (req, res) => {
    try {
        ...
        const def = getCardDefs().find((c) => c.key === cardKey);
        if (!def)
...
router.get("/admin/analytics", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
...
const getTreasuryTokenBalance = async (connection, mint, owner) => {
    try {
        ...
        console.error("getTreasuryTokenBalance RPC error:", err?.message || err);
...
const isSignatureUsedElsewhere = async (signature) => {
    const variants = [signature, `${signature}_site`, `${signature}_referrer`];
    const used = await prisma.payment
    ...
    try {
        ...
        const results = await Promise.all(batch.map(async (sg) => {
            try {
                ...
                if (await isSignatureUsedElsewhere(sg.signature))
                    return null;
...
router.post("/admin/distribution/confirm", authenticateJWT, async (req, res) => {
    try {
        ...
        const alreadyRecorded = new Set((await prisma.distributionRecord.findMany({
...
router.post("/ico/purchase", authenticateJWT, async (req, res) => {
    try {
        ...
        if (signature) {
            const existing = await prisma.icoPurchase.findUnique({ where: { txHash: signature } });
            if (existing)
        ...
        let purchase;
        try {
            purchase = await prisma.icoPurchase.create({
...
router.get("/admin/ico/pending", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        const rows = await prisma.icoPurchase.findMany({
        ...
        for (const r of rows) {
            ...
            const entry = byWallet.get(wallet) || { walletAddress: wallet, tokenAmount: 0, purchaseIds: [] };
...
router.post("/admin/ico/distribute", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        ...
        const cfg = getTokenConfig();
        if (!cfg.mint)
            return res.status(400).json({ message: "عقد التوكن غير مربوط بعد (من إعدادات التوكن)" });
        const connection = new Connection(getRpcUrl(), "confirmed");
        const mint = new PublicKey(cfg.mint);
        const results = [];
        ...
        for (const item of deliver) {
            ...
            const purchaseIds = Array.isArray(item.purchaseIds) ? item.purchaseIds.map(Number) : [];
        ...
        for (let i = 0; i < deliver.length; i++) {
            ...
            if (r && r.ok && r.purchaseIds.length) {
                await prisma.icoPurchase.updateMany({
...
router.post("/admin/card-image", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        ...
        if (!m)
            return res.status(400).json({ message: "تنسيق صورة غير صالح — الصيغ: png/jpg/webp/gif" });
        const ext = m[1].toLowerCase() === "jpg" ? "jpg" : m[1].toLowerCase();
        const buf = Buffer.from(m[2], "base64");
        if (buf.length > 2 * 1024 * 1024)
            return res.status(413).json({ message: "الصورة كبيرة جداً — الحد 2MB" });
...
router.post("/admin/card-image/delete", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        const { url } = req.body || {};
        if (typeof url !== "string" || !url.startsWith("/uploads/cards/"))
