...
router.get("/admin/achievements/search", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        ...
        const enriched = await Promise.all(users.map(async (u) => {
            const [latestBonus, approvedTasks, games7, gamesTotal, activeFriends] = await Promise.all([
                prisma.dailyBonus.findFirst({ where: { userId: u.id }, orderBy: { claimedAt: "desc" }, select: { streakDay: true } }),
                prisma.socialTask.count({ where: { userId: u.id, status: "approved" } }),
                prisma.gamePlayRecord.count({ where: { userId: u.id, createdAt: { gte: new Date(Date.now() - 7 * 864e5) } } }),
                prisma.gamePlayRecord.count({ where: { userId: u.id } }),
...
router.get("/admin/achievements", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        ...
        return res.json(rows.map((a) => ({ ...a, reward: Number(a.reward), createdAt: a.createdAt })));
    }
    catch (error) {
...
router.post("/admin/achievements", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        ...
        if (!uId || !type)
            return res.status(400).json({ message: "بيانات الإنجاز ناقصة — اختر المستخدم ونوع الإنجاز أولاً" });
        if (!Number.isFinite(rid) || rid <= 0 || rid > 1000000)
            return res.status(400).json({ message: "قيمة المكافأة غير صالحة (من 1 إلى 1,000,000)" });
        ...
        const achievement = await prisma.userAchievement.create({
            data: {
                ...
                grantedBy: req.user?.walletAddress || "admin",
            },
        });
...
router.delete("/admin/achievements/:id", authenticateJWT, async (req, res) => {
    try {
        if (!isAdmin(req, res))
            return;
        await prisma.userAchievement.delete({ where: { id: Number(req.params.id) } });
...
router.get("/me/achievements", authenticateJWT, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
