...
const pickWheelSegment = () => {
    const { segments } = getWheel();
    const totalWeight = segments.reduce((sum, sg) => sum + Math.max(0, sg.weight || 1), 0);
...
const fmtLock = (sec) => {
    ...
    if (sec >= 60)
        return `${Math.ceil(sec / 60)} دقيقة`;
    return `${sec} ثانية`;
};
// ⚡ مضاعف موحد متدرج بمستوى اللعبة — ينطبق على أرباح الألعاب فقط (L1=1.00, L2=1.15, L3=1.30 … سقف 5x)
const getGameMultiplier = (level) => Math.min(5, 1 + (Math.max(1, level) - 1) * 0.15);
...
router.get("/status", authenticateJWT, async (req, res) => {
    try {
        ...
        let progress = await prisma.gameProgress.findUnique({ where: { userId } });
        if (!progress) {
            progress = await prisma.gameProgress.create({ data: { userId } });
        }
        const startOfDay = new Date();
        startOfDay.setHours(0, 0, 0, 0);
        const todayEarned = { wheel: 0, xo: 0, catch: 0 };
        const cooldowns = { wheel: 0, xo: 0, catch: 0 };
        const games = ["wheel", "xo", "catch"];
        for (const g of games) {
            const [agg, last] = await Promise.all([
                prisma.gamePlayRecord.aggregate({
                    _sum: { reward: true },
                    where: { userId, game: g, createdAt: { gte: startOfDay } },
                }),
                prisma.gamePlayRecord.findFirst({
                    where: { userId, game: g },
...
// ==========================================
router.post("/wheel/spin", authenticateJWT, async (req, res) => {
    try {
        const userId = req.user.id;
        const user = await prisma.user.findUnique({ where: { id: userId } });
...
router.post("/result", authenticateJWT, async (req, res) => {
    try {
        ...
        let scoreVal = 0;
        if (game === "wheel") {
            if (spinToken == null || segment == null)
                return res.status(400).json({ message: "طلب عجلة غير مكتمل" });
            let payload;
            try {
            ...
            if (score == null)
                return res.status(400).json({ message: "النتيجة مفقودة" });
            if (cfg.maxScore != null && score > cfg.maxScore)
                return res.status(400).json({ message: "نتيجة غير صحيحة" });
            base = score * cfg.factor;
            scoreVal = score;
            // جولة صفرية (بدون نقاط) لا تحسب كجولة ولا تطبق كولدون
            if (score <= 0)
                return res.json({ reward: 0, balance: Number(user.balance), xpGained: 0, gameLevel: 0, gameXp: 0, leveledUp: false });
        }
        // ⏱️ كولدون: من آخر جولة مسجلة لنفس اللعبة
        const cfgSec = game === "wheel" ? getWheel().cooldownSec : cfg.cooldownSec;
        const last = await prisma.gamePlayRecord.findFirst({
            where: { userId, game },
            orderBy: { createdAt: "desc" },
        });
        if (last) {
        ...
        const [dayByGameAgg, dayTotalAgg] = await Promise.all([
            prisma.gamePlayRecord.aggregate({ _sum: { reward: true }, where: { userId, game, createdAt: { gte: startOfDay } } }),
            prisma.gamePlayRecord.aggregate({ _sum: { reward: true }, where: { userId, createdAt: { gte: startOfDay } } }),
        ]);
        ...
        const [updatedUser] = await prisma.$transaction(async (tx) => {
            const u = await tx.user.update({ where: { id: userId }, data: { balance: { increment: final } } });
        ...
        try {
            await awardActivity(userId, "xpGame");
        }
        catch (e) {
            console.error("game activity error:", e);
        }
        return res.json({
            reward: final,
...
router.get("/history", authenticateJWT, async (req, res) => {
    try {
        ...
        const records = await prisma.gamePlayRecord.findMany({
            where: { userId },
            orderBy: { createdAt: "desc" },
            take: limit,
        });
