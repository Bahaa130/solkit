...
router.post("/resolve-referral", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const code = String(req.body?.referralCode || "").trim();
    if (!code) return res.status(400).json({ message: "أدخل كود الإحالة أولاً" });
    ...
    if (!referrer) return res.status(400).json({ message: "كود الإحالة غير صحيح، تأكد منه وأعد المحاولة" });
    if (referrer.id === req.user!.id) return res.status(400).json({ message: "لا يمكنك إدخال كود الإحالة الخاص بك!" });
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      ...
      if (!/expired|block he?ight exceeded/i.test(msg)) {
        ...
        if (attempt === 0) console.warn(`fetchTransactionWithRetry (${signature.slice(0, 8)}…): ${msg}`);
      }
    }
...
router.post("/admin/settings", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  ...
  // ==========================================
// 🏅 مسارات الإنجازات — يمنحها المدير يدوياً (الدفع: رصيد داخلي فوري)
// ==========================================

// 🔍 بحث المدير عن مستخدمين + عدّادات مرجعية حية (لمراجعة تقدم المستخدم قبل الاعتماد)
router.get("/admin/achievements/search", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    ...
    const users = await prisma.user.findMany({
      ...
      take: 15,
      orderBy: { createdAt: "desc" },
      select: { id: true, name: true, email: true, walletAddress: true, referralCode: true, activationStatus: true, currentLevel: true, balance: true },
    });
    const enriched = await Promise.all(users.map(async (u) => {
      const [latestBonus, approvedTasks, games7, gamesTotal, activeFriends] = await Promise.all([
        (prisma as any).dailyBonus.findFirst({ where: { userId: u.id }, orderBy: { claimedAt: "desc" }, select: { streakDay: true } } as any),
        (prisma as any).socialTask.count({ where: { userId: u.id, status: "approved" } } as any),
        (prisma as any).gamePlayRecord.count({ where: { userId: u.id, createdAt: { gte: new Date(Date.now() - 7 * 864e5) } } } as any),
        (prisma as any).gamePlayRecord.count({ where: { userId: u.id } } as any),
        prisma.user.count({ where: { referrerId: u.id, activationStatus: "active" } }),
      ]);
      return { ...u, balance: Number(u.balance), streakDay: latestBonus?.streakDay || 0, approvedTasks, games7, gamesTotal, activeFriends };
    }));
    return res.json(enriched);
  } catch (error) {
    console.error("Achievements search error:", error);
    return res.status(500).json({ message: "فشل البحث عن المستخدمين" });
  }
});

// 📋 قائمة إنجازات المدير (كل المسجلة أو حسب مستخدم)
router.get("/admin/achievements", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    ...
    const rows = await (prisma as any).userAchievement.findMany({
      ...
      orderBy: { createdAt: "desc" },
      take: 200,
      include: { user: { select: { id: true, name: true, email: true, walletAddress: true, referralCode: true } } },
    } as any);
    return res.json(rows.map((a: any) => ({ ...a, reward: Number(a.reward), createdAt: a.createdAt })));
  } catch (error) {
    console.error("Achievements list error:", error);
    return res.status(500).json({ message: "فشل جلب قائمة الإنجازات" });
  }
});

// 🎁 منح إنجاز يدوياً + إضافة المكافأة إلى رصيد المستخدم الداخلي فوراً
router.post("/admin/achievements", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const { userId, type, target, reward, note } = req.body || {};
    const uId = Number(userId);
    const rid = Number(reward);
    if (!uId || !type) return res.status(400).json({ message: "بيانات الإنجاز ناقصة — اختر المستخدم ونوع الإنجاز أولاً" });
    if (!Number.isFinite(rid) || rid <= 0 || rid > 1_000_000) return res.status(400).json({ message: "قيمة المكافأة غير صالحة (من 1 إلى 1,000,000)" });
    const user = await prisma.user.findUnique({ where: { id: uId } });
    if (!user) return res.status(404).json({ message: "المستخدم غير موجود" });

    const achievement = await (prisma as any).userAchievement.create({
      data: {
        userId: user.id,
        type: String(type),
        target: String(target || type || "").trim() || String(type),
        reward: rid,
        note: note ? String(note).trim().slice(0, 300) : null,
        grantedBy: (req as AuthenticatedRequest).user?.walletAddress || "admin",
      },
    } as any);
    await prisma.user.update({ where: { id: user.id }, data: { balance: { increment: rid } } });
    return res.json({ message: "تم منح الإنجاز وإضافة المكافأة إلى رصيد المستخدم ✅", achievement: { ...achievement, reward: Number(achievement.reward) } });
  } catch (error) {
    console.error("Achievements grant error:", error);
    return res.status(500).json({ message: "فشل منح الإنجاز" });
  }
});

// 🗑️ حذف إنجاز (تصحيح خطأ — لا يسحب الرصيد تلقائياً)
router.delete("/admin/achievements/:id", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    await (prisma as any).userAchievement.delete({ where: { id: Number(req.params.id) } });
    return res.json({ message: "تم حذف الإنجاز" });
  } catch (error) {
    console.error("Achievements delete error:", error);
    return res.status(500).json({ message: "تعذر حذف الإنجاز" });
  }
});

// 🏅 إنجازات المستخدم الحالي (تُعرض له في صفحة المستويات)
router.get("/me/achievements", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    return res.status(500).json({ message: "فشل جلب إنجازاتك" });
  }
});
