...
router.post("/login-challenge", async (req: Request, res: Response) => {
  try {
    ...
    challengeStore.set(walletAddress, { nonce, expires: Date.now() + 15 * 60_000 });
...
const getCardIncome = async (userId: number): Promise<number> => {
  ...
  let total = 0;
  for (const d of defs) {
    const lvl = byKey.get(d.key) || 0;
    if (lvl > 0) total += Number(d.reward || 0) * lvl;
  }
  return total;
};
...
router.get("/mining-status", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!activeSession) return res.json({ status: "stopped", miningRate: currentRate, cardIncome, timeLeft: 0, pendingMinedAmount: 0 });
    ...
    // 🔄 جلسة نشطة: أرباح لحظية = الزمن المنقضي فعلياً × معدل الجلسة (يشمل ترقيات الكروت)
    const secondsPassed = Math.floor((now.getTime() - new Date(activeSession.startedAt).getTime()) / 1000);
    const pendingMinedAmount = (secondsPassed * Number(activeSession.miningRate)) / 3600;
    return res.json({
      status: "active",
      miningRate: Number(activeSession.miningRate),
      cardIncome,
...
router.get("/cards/list", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { id: userId }, select: { balance: true, currentLevel: true } });
    return res.json({
      cards,
      totalIncome,
      balance: Number(user?.balance || 0),
      baseRate: user ? rateForLevel(user.currentLevel || 1) : 0,
    });
  } catch (e) {
...
router.post("/cards/upgrade", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const def = getCardDefs().find((c: CardDef) => c.key === cardKey);
    ...
    const cost = Math.round((Number(def.baseCost) || 0) * Math.pow(Number(def.costGrowth) || 1, level) * 100) / 100;
...
const fetchIcoTransaction = async (signature: string): Promise<any | null> => {
  try {
...
const extractTreasuryPayment = (tx: any): { from: string; lamports: number } | null => {
  if (!tx || !tx.meta || tx.meta.err) return null;
...
const findEligibleIcoPayment = async (
  solanaRpcUrl: string,
  userWallet: string,
  ...
  try {
    ...
    const treasury = getTokenConfig().treasury;
    const sigsResp = await withTimeout(
      connection.getSignaturesForAddress(new PublicKey(treasury), { limit: 30 }),
      10000,
    );
    if (!sigsResp) return null;
    const signatures = (sigsResp as any[]).filter((sg) => !sg.err);
    const LIMIT = Math.min(signatures.length, 10);
    const batch = signatures.slice(0, LIMIT);
    const results = await Promise.all(
      batch.map(async (sg): Promise<{ signature: string; lamports: number } | null> => {
        try {
          const tx = await fetchIcoTransaction(sg.signature);
          const p = tx ? extractTreasuryPayment(tx) : null;
          if (!p || p.from !== userWallet) return null;
...
const scheduleIcoAutoConfirm = (
  ...
  const attempt = async () => {
    try {
      const p: any = await (prisma as any).icoPurchase.findUnique({ where: { id: purchaseId } });
      if (!p || p.status !== "pending") return; // تأكد/أُلغي يدوياً → نتوقف
...
router.post("/admin/ico/confirm", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const id = Number(req.body?.id);
    if (!Number.isInteger(id)) return res.status(400).json({ message: "معرّف العملية غير صالح" });
    const p = await (prisma as any).icoPurchase.findUnique({ where: { id } });
    if (!p) return res.status(404).json({ message: "العملية غير موجودة" });
    await (prisma as any).icoPurchase.update({ where: { id }, data: { status: "purchased" } });
...
router.post("/admin/ico/unconfirm", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const id = Number(req.body?.id);
    if (!Number.isInteger(id)) return res.status(400).json({ message: "معرّف العملية غير صالح" });
...
router.post("/admin/ico/add-purchase", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const s: SiteSettings = getSettings();
    const ico = s?.ico;
    if (!ico) return res.status(400).json({ message: "إعدادات الاكتتاب غير موجودة" });
    ...
    if (!user) return res.status(404).json({ message: "لا يوجد حساب بهذا البريد — تحقق من البريد المسجَّل بالمشترك" });
    if (!Number.isFinite(amount) || amount <= 0) return res.status(400).json({ message: "المبلغ غير صالح" });
    if (amount < ico.minSOL) return res.status(400).json({ message: `الحد الأدنى للمشاركة ${ico.minSOL} SOL` });
    if (amount > ico.maxSOL) return res.status(400).json({ message: `الحد الأقصى للمشاركة ${ico.maxSOL} SOL` });
    if (txHash) {
      const dup = await (prisma as any).icoPurchase.findUnique({ where: { txHash } });
      if (dup) return res.status(400).json({ message: "هذا الإيصال مسجَّل مسبقاً" });
    }
...
router.post("/admin/reset-levels", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
