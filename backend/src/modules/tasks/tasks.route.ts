...
router.post("/verify", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ message: "صيغة الطلب غير صالحة — تحقق من اسم الحساب" });

    const userId = req.user!.id;
    const { channelId, socialUsername } = parsed.data;

    const channel = await prisma.socialChannel.findUnique({ where: { id: channelId } });
    if (!channel || !channel.active) return res.status(400).json({ message: "هذا الحساب المجتمعي غير متاح حالياً" });
...
router.get("/admin/channels", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.post("/admin/channels", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const parsed = channelSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ message: "صيغة بيانات الحساب غير صالحة" });
...
// تعديل حساب مجتمعي
router.put("/admin/channels/:id", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ message: "معرّف الحساب غير صالح" });
    const parsed = channelSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ message: "صيغة بيانات الحساب غير صالحة" });
    const { title, platform, link, reward, active, sortOrder } = parsed.data;
...
// حذف حساب مجتمعي
router.delete("/admin/channels/:id", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const id = Number(req.params.id);
    if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ message: "معرّف الحساب غير صالح" });
...
router.get("/admin/pending", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
...
router.post("/admin/approve", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ message: "صيغة الطلب غير صالحة" });

    const task = await prisma.socialTask.findUnique({
      where: { id: parsed.data.taskId },
      include: { channel: true, user: true },
    });
    if (!task) return res.status(404).json({ message: "طلب التحقق غير موجود" });
    if (task.status === "approved") return res.status(400).json({ message: "تمت الموافقة على هذا الطلب مسبقاً" });
...
router.post("/admin/reject", authenticateJWT, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!isAdmin(req, res)) return;
    const parsed = taskIdSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ message: "صيغة الطلب غير صالحة" });
